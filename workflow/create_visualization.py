...
for index,row in data.iterrows():
    ...
    g_hex = "{:02x}".format(int(g_num))
    b_hex = "{:02x}".format(int(b_num))
    colors.append('#00'+g_hex+b_hex)

plt.scatter(x, y, c=colors)
plt.xlabel("GDP Per Capita")
plt.ylabel("Child Mortality Rate")
green_patch = mpatches.Patch(color='#00FF00', label='Year: '+str(max(years)))
blue_patch = mpatches.Patch(color='#0000FF', label='Year: '+str(min(years)))
